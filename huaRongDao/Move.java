...
public class Move {
	...
	public Move(Board oldBoard, Block oldBlock, int deltaXPos, int deltaYPos, Block newBlock) {
		...
		if (deltaXPos == 0 && deltaYPos == -1) {
			...
			if ((oldBoard.getValidMovesMask(MoveType.LEFT) & oldBlock.bitImage) == oldBlock.bitImage) {
			...
			if ((oldBoard.getValidMovesMask(MoveType.RIGHT) & oldBlock.bitImage) == oldBlock.bitImage) {
